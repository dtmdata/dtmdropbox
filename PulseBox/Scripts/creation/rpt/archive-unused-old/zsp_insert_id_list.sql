...
sp_insert_id_list()
D.Durland 20150203:
Parse comma-sep input string of values, and insert them to a pre-created GTT, gtt_id_list.
Assumes perfectly-formed comma separated string (No leading, trailing, or multiple commas in a row.)
Should only be called from *within* another of our pbox st.procs.
*/
create or replace procedure sp_insert_id_list (IN_dept_id_list  IN varchar2)

AS
v_id_list varchar2(255) := '' ;  
...
LOOP
  ...
  if v_next_comma <= 1 then
    if length(v_id_list) < 1 then
      ...
      exit; -- this should have been last ID, with no trailing comma, so exit LOOP!
    end if;
  end if;
