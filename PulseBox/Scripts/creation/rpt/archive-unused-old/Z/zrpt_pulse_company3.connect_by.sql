...
did we decide to stick with JUST that top level(entire company), or treat the emp_id who is logged in as "the company" ?
did we end up deciding to only go with a single dept?  or to let the emp_id person choose from however many depts which might be under his level of mgr/emp org?
...
Ebadi, Abdul 15:11
Just top level company... no report by emp_id yet.
FOr this one, just two reports: entire company, or a dept they choose by name from list.
Durland, Dave 15:13
so, choose from a unique Dept list for all depts at the user's level, and below.  and then calculate pulse from those emp_ids
??
Ebadi, Abdul 15:13
Exactly... depts at user level and below
